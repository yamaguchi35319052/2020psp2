...
- `ave_online`, `var_online`の引数に`n`が無い件について，
  - 関数は汎用（流用）可能にしたほうがいいです．どこでも使える関数にするという意味です．その意味で引数に`n`があるべきです．
  - `n`をグローバル変数にして，どの関数でも見えるようにしているようですが，一見便利ですが，逆に言うとどの関数でも見えており，どの関数で値を上書きしても他にそれが反映されてしまうという危険なコーディングです．やめてください．
